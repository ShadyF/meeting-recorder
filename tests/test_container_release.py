...
def test_validate_command_and_output_safety() -> None:
    ...
    with tempfile.TemporaryDirectory() as directory:
        output = Path(directory) / "output"
        result = release.main(["validate", "--tag", "v0.4.3", "--revision", "A" * 40, "--project-root", str(ROOT), "--github-output", str(output)])
        assert result == 0
        assert output.read_text() == f"version=0.4.3\nversion_tag=0.4.3\nsha_tag=sha-{'a' * 40}\nrevision={'a' * 40}\nprerelease=false\nsource={release.SOURCE}\n"
        assert "version_tag=v" not in output.read_text()
        link = Path(directory) / "link"
        link.symlink_to(output)
        assert release.main(["validate", "--tag", "v0.4.3", "--revision", "a" * 40, "--project-root", str(ROOT), "--github-output", str(link)]) == 2
